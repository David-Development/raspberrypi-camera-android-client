...
public class MjpegViewDefault extends AbstractMjpegView {
    ...
    class MjpegViewThread extends Thread {
        ...
        private SparseArray<Rect> rectCache = new SparseArray<>();
        ...
        private Rect destRect(int bmw, int bmh) {
            ...
            if (displayMode == MjpegViewDefault.SIZE_STANDARD) {
            ...
            } else if (displayMode == MjpegViewDefault.SIZE_BEST_FIT) {
                ...
                tempy = (dispHeight / 2) - (bmh / 2);
                rect = new Rect(tempx, tempy, bmw + tempx, bmh + tempy);
            } else if (displayMode == MjpegViewDefault.SIZE_FULLSCREEN) {
                rect = new Rect(0, 0, dispWidth, dispHeight);
            }
            rectCache.put(key, rect);
            return rect;
        }
                    ...
                    try {
                        ...
                        if (showFps) {
                            ...
                            if ((currentTime - start) >= 1000) {
                                fps = String.valueOf(frameCounter*1000 / (currentTime - start)) + "fps";
                                frameCounter = 0;
                                start = currentTime;
                                ovl = makeFpsOverlay(overlayPaint, fps);
                            }
